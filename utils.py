...
def train_dev_test_split(data, label, train_frac, dev_frac, rsplit):

    dev_test_frac = 1 - train_frac
    x_train, x_dev_test, y_train, y_dev_test = train_test_split(
        data, label, test_size=dev_test_frac, shuffle=True, random_state=rsplit
    )
    x_test, x_dev, y_test, y_dev = train_test_split(
        x_dev_test, y_dev_test, test_size=(dev_frac) / dev_test_frac, shuffle=True, random_state=rsplit
    )
...
def tune_and_save(clf, x_train, y_train, x_dev, y_dev, metric, h_param_comb, model_path):
    ...
    if model_path == None:
        ...
        model_path = "./model/"+model_type + "_" + best_param_config + ".joblib"
